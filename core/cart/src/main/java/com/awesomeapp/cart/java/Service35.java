...
public class Service35 {
    private final Map<String, DataModel16> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel16 model) {
    ...
    public Optional<DataModel16> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel16> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel16> filterByActive(boolean active) {
