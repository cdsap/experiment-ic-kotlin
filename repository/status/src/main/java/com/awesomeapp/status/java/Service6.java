package com.awesomeapp.status.java;
...
public class Service6 {
    private final Map<String, DataModel5> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel5 model) {
    ...
    public Optional<DataModel5> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel5> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel5> filterByActive(boolean active) {
