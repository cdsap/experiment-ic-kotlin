...
public class Service8 {
    private final Map<String, DataModel7> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel7 model) {
    ...
    public Optional<DataModel7> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel7> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel7> filterByActive(boolean active) {
