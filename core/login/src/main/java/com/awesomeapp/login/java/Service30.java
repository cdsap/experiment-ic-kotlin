...
public class Service30 {
    private final Map<String, DataModel29> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel29 model) {
    ...
    public Optional<DataModel29> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel29> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel29> filterByActive(boolean active) {
