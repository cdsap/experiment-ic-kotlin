...
public class Service29 {
    private final Map<String, DataModel10> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel10 model) {
    ...
    public Optional<DataModel10> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel10> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel10> filterByActive(boolean active) {
