...
public class Container5457 {
    ...
    public <T> Container5457 registerSingleton(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.SINGLETON));
        return this;
    }

    public <T> Container5457 registerTransient(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.TRANSIENT));
        return this;
    }

    public <T> Container5457 registerScoped(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.SCOPED));
        return this;
    }

    public <I, T extends I> Container5457 bind(Class<I> iface, Class<T> impl) {
        interfaceBindings.put(iface, impl);
        return this;
    }

    public <T> Container5457 registerInstance(Class<T> type, T instance) {
