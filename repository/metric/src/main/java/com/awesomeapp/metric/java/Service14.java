...
public class Service14 {
    private final Map<String, DataModel13> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel13 model) {
    ...
    public Optional<DataModel13> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel13> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel13> filterByActive(boolean active) {
