package com.awesomeapp.forecast.java;
...
public class Service32 {
    private final Map<String, DataModel31> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel31 model) {
    ...
    public Optional<DataModel31> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel31> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel31> filterByActive(boolean active) {
