...
public class Service28 {
    private final Map<String, DataModel27> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel27 model) {
    ...
    public Optional<DataModel27> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel27> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel27> filterByActive(boolean active) {
