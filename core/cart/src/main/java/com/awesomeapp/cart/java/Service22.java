...
public class Service22 {
    private final Map<String, DataModel3> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel3 model) {
    ...
    public Optional<DataModel3> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel3> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel3> filterByActive(boolean active) {
