package com.awesomeapp.todo.java;
...
public class Service40 {
    private final Map<String, DataModel39> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel39 model) {
    ...
    public Optional<DataModel39> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel39> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel39> filterByActive(boolean active) {
