package com.awesomeapp.checkout.java;
...
public class Service24 {
    private final Map<String, DataModel23> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel23 model) {
    ...
    public Optional<DataModel23> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel23> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel23> filterByActive(boolean active) {
