...
public class Service50 {
    private final Map<String, DataModel49> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel49 model) {
    ...
    public Optional<DataModel49> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel49> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel49> filterByActive(boolean active) {
