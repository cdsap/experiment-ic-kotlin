package com.awesomeapp.file.java;
...
public class Service36 {
    private final Map<String, DataModel35> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel35 model) {
    ...
    public Optional<DataModel35> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel35> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel35> filterByActive(boolean active) {
