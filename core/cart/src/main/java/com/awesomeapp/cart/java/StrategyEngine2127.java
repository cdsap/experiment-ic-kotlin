...
public class StrategyEngine2127<T, R> {
    ...
    public StrategyEngine2127<T, R> register(String name, Strategy<T, R> strategy) {
        registry.put(name, strategy);
        if (defaultStrategy == null) defaultStrategy = name;
        return this;
    }

    public StrategyEngine2127<T, R> setDefault(String name) {
        if (!registry.containsKey(name)) throw new IllegalArgumentException("Strategy not found: " + name);
        this.defaultStrategy = name;
        return this;
    }

    public StrategyEngine2127<T, R> withCombiner(BiFunction<List<StrategyResult<R>>, T, R> combiner) {
