...
public class Service39 {
    private final Map<String, DataModel20> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel20 model) {
    ...
    public Optional<DataModel20> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel20> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel20> filterByActive(boolean active) {
