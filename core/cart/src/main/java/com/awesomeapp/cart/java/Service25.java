...
public class Service25 {
    private final Map<String, DataModel6> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel6 model) {
    ...
    public Optional<DataModel6> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel6> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel6> filterByActive(boolean active) {
