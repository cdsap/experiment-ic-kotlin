...
public class Service31 {
    private final Map<String, DataModel12> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel12 model) {
    ...
    public Optional<DataModel12> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel12> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel12> filterByActive(boolean active) {
