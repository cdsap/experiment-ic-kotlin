...
public class Container2357 {
    ...
    public <T> Container2357 registerSingleton(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.SINGLETON));
        return this;
    }

    public <T> Container2357 registerTransient(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.TRANSIENT));
        return this;
    }

    public <T> Container2357 registerScoped(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.SCOPED));
        return this;
    }

    public <I, T extends I> Container2357 bind(Class<I> iface, Class<T> impl) {
        interfaceBindings.put(iface, impl);
        return this;
    }

    public <T> Container2357 registerInstance(Class<T> type, T instance) {
