...
public class Service46 {
    private final Map<String, DataModel45> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel45 model) {
    ...
    public Optional<DataModel45> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel45> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel45> filterByActive(boolean active) {
