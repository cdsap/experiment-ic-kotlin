package com.awesomeapp.alarm.java;
...
public class Service12 {
    private final Map<String, DataModel11> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel11 model) {
    ...
    public Optional<DataModel11> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel11> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel11> filterByActive(boolean active) {
