...
public abstract class Specification5334<T> {

    public abstract boolean isSatisfiedBy(T candidate);
    public abstract String describe();

    public Specification5334<T> and(Specification5334<T> other) {
        return new AndSpecification<>(this, other);
    }

    public Specification5334<T> or(Specification5334<T> other) {
        return new OrSpecification<>(this, other);
    }

    public Specification5334<T> not() {
    ...
    public static <T> Specification5334<T> of(String description, Predicate<T> predicate) {
        return new PredicateSpecification<>(description, predicate);
    }

    public static <T, V extends Comparable<V>> Specification5334<T> greaterThan(String field, Function<T, V> extractor, V value) {
        return of(field + " > " + value, t -> extractor.apply(t).compareTo(value) > 0);
    }

    public static <T, V extends Comparable<V>> Specification5334<T> lessThan(String field, Function<T, V> extractor, V value) {
        return of(field + " < " + value, t -> extractor.apply(t).compareTo(value) < 0);
    }

    public static <T> Specification5334<T> equalTo(String field, Function<T, ?> extractor, Object value) {
        return of(field + " == " + value, t -> Objects.equals(extractor.apply(t), value));
    }

    public static <T> Specification5334<T> in(String field, Function<T, ?> extractor, Collection<?> values) {
        return of(field + " in " + values, t -> values.contains(extractor.apply(t)));
    }

    private static class PredicateSpecification<T> extends Specification5334<T> {
    ...
    private static class AndSpecification<T> extends Specification5334<T> {
        private final Specification5334<T> left, right;
        AndSpecification(Specification5334<T> left, Specification5334<T> right) { this.left = left; this.right = right; }
    ...
    private static class OrSpecification<T> extends Specification5334<T> {
        private final Specification5334<T> left, right;
        OrSpecification(Specification5334<T> left, Specification5334<T> right) { this.left = left; this.right = right; }
        @Override public boolean isSatisfiedBy(T candidate) { return left.isSatisfiedBy(candidate) || right.isSatisfiedBy(candidate); }
        @Override public String describe() { return "(" + left.describe() + " OR " + right.describe() + ")"; }
    }

    private static class NotSpecification<T> extends Specification5334<T> {
        private final Specification5334<T> spec;
        NotSpecification(Specification5334<T> spec) { this.spec = spec; }
