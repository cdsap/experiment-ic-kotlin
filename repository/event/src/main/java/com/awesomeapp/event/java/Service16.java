...
public class Service16 {
    private final Map<String, DataModel15> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel15 model) {
    ...
    public Optional<DataModel15> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel15> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel15> filterByActive(boolean active) {
