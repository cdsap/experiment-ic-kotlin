...
public class Container2057 {
    ...
    public <T> Container2057 registerSingleton(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.SINGLETON));
        return this;
    }

    public <T> Container2057 registerTransient(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.TRANSIENT));
        return this;
    }

    public <T> Container2057 registerScoped(Class<T> type, Supplier<T> factory) {
        registrations.put(type, new Registration<>(type, factory, Lifecycle.SCOPED));
        return this;
    }

    public <I, T extends I> Container2057 bind(Class<I> iface, Class<T> impl) {
        interfaceBindings.put(iface, impl);
        return this;
    }

    public <T> Container2057 registerInstance(Class<T> type, T instance) {
