package com.awesomeapp.identity.java;
...
public class Service20 {
    private final Map<String, DataModel19> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel19 model) {
    ...
    public Optional<DataModel19> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel19> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel19> filterByActive(boolean active) {
