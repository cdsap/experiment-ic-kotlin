...
public class Service48 {
    private final Map<String, DataModel47> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel47 model) {
    ...
    public Optional<DataModel47> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel47> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel47> filterByActive(boolean active) {
