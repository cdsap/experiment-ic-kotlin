package com.awesomeapp.push.java;
...
public class Service26 {
    private final Map<String, DataModel25> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel25 model) {
    ...
    public Optional<DataModel25> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel25> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel25> filterByActive(boolean active) {
