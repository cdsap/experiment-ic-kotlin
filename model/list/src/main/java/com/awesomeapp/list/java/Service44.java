...
public class Service44 {
    private final Map<String, DataModel43> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel43 model) {
    ...
    public Optional<DataModel43> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel43> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel43> filterByActive(boolean active) {
