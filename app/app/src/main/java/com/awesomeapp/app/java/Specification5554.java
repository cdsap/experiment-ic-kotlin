...
public abstract class Specification5554<T> {

    public abstract boolean isSatisfiedBy(T candidate);
    public abstract String describe();

    public Specification5554<T> and(Specification5554<T> other) {
        return new AndSpecification<>(this, other);
    }

    public Specification5554<T> or(Specification5554<T> other) {
        return new OrSpecification<>(this, other);
    }

    public Specification5554<T> not() {
    ...
    public static <T> Specification5554<T> of(String description, Predicate<T> predicate) {
        return new PredicateSpecification<>(description, predicate);
    }

    public static <T, V extends Comparable<V>> Specification5554<T> greaterThan(String field, Function<T, V> extractor, V value) {
        return of(field + " > " + value, t -> extractor.apply(t).compareTo(value) > 0);
    }

    public static <T, V extends Comparable<V>> Specification5554<T> lessThan(String field, Function<T, V> extractor, V value) {
        return of(field + " < " + value, t -> extractor.apply(t).compareTo(value) < 0);
    }

    public static <T> Specification5554<T> equalTo(String field, Function<T, ?> extractor, Object value) {
        return of(field + " == " + value, t -> Objects.equals(extractor.apply(t), value));
    }

    public static <T> Specification5554<T> in(String field, Function<T, ?> extractor, Collection<?> values) {
        return of(field + " in " + values, t -> values.contains(extractor.apply(t)));
    }

    private static class PredicateSpecification<T> extends Specification5554<T> {
    ...
    private static class AndSpecification<T> extends Specification5554<T> {
        private final Specification5554<T> left, right;
        AndSpecification(Specification5554<T> left, Specification5554<T> right) { this.left = left; this.right = right; }
    ...
    private static class OrSpecification<T> extends Specification5554<T> {
        private final Specification5554<T> left, right;
        OrSpecification(Specification5554<T> left, Specification5554<T> right) { this.left = left; this.right = right; }
        @Override public boolean isSatisfiedBy(T candidate) { return left.isSatisfiedBy(candidate) || right.isSatisfiedBy(candidate); }
        @Override public String describe() { return "(" + left.describe() + " OR " + right.describe() + ")"; }
    }

    private static class NotSpecification<T> extends Specification5554<T> {
        private final Specification5554<T> spec;
        NotSpecification(Specification5554<T> spec) { this.spec = spec; }
