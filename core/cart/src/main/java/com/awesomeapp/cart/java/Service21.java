...
public class Service21 {
    private final Map<String, DataModel2> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel2 model) {
    ...
    public Optional<DataModel2> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel2> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel2> filterByActive(boolean active) {
