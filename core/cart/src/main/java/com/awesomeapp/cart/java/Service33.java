...
public class Service33 {
    private final Map<String, DataModel14> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel14 model) {
    ...
    public Optional<DataModel14> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel14> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel14> filterByActive(boolean active) {
