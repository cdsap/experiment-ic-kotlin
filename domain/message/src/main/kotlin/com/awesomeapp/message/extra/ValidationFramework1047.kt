...
class ValidationFramework1047<T : Any> {
    ...
    fun addRule(rule: Rule<T>): ValidationFramework1047<T> { rules.add(rule); return this }

    fun <V> field(
        name: String, field: String, extractor: (T) -> V,
        predicate: (V) -> Boolean, message: String, code: String,
        severity: Severity = Severity.ERROR
    ): ValidationFramework1047<T> {
        rules.add(FieldRule(name, field, extractor, predicate, message, code, severity))
        return this
    }

    fun addGroup(group: String, vararg groupRules: Rule<T>): ValidationFramework1047<T> {
