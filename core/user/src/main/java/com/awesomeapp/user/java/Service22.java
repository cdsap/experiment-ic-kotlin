...
public class Service22 {
    private final Map<String, DataModel21> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel21 model) {
    ...
    public Optional<DataModel21> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel21> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel21> filterByActive(boolean active) {
