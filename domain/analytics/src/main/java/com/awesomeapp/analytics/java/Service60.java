...
public class Service60 {
    private final Map<String, DataModel59> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel59 model) {
    ...
    public Optional<DataModel59> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel59> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel59> filterByActive(boolean active) {
