...
public class Service23 {
    private final Map<String, DataModel4> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel4 model) {
    ...
    public Optional<DataModel4> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel4> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel4> filterByActive(boolean active) {
