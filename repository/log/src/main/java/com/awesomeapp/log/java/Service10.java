package com.awesomeapp.log.java;
...
public class Service10 {
    private final Map<String, DataModel9> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel9 model) {
    ...
    public Optional<DataModel9> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel9> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel9> filterByActive(boolean active) {
