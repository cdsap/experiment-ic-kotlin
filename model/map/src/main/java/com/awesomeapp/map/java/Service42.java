package com.awesomeapp.map.java;
...
public class Service42 {
    private final Map<String, DataModel41> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel41 model) {
    ...
    public Optional<DataModel41> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel41> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel41> filterByActive(boolean active) {
