...
public class Service38 {
    private final Map<String, DataModel37> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel37 model) {
    ...
    public Optional<DataModel37> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel37> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel37> filterByActive(boolean active) {
