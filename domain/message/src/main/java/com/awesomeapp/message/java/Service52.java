...
public class Service52 {
    private final Map<String, DataModel51> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel51 model) {
    ...
    public Optional<DataModel51> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel51> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel51> filterByActive(boolean active) {
