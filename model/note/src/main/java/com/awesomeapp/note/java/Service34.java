...
public class Service34 {
    private final Map<String, DataModel33> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel33 model) {
    ...
    public Optional<DataModel33> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel33> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel33> filterByActive(boolean active) {
