...
public class Service36 {
    private final Map<String, DataModel17> cache = new HashMap<>();
    private final List<String> processedIds = new ArrayList<>();
    private int operationCount = 0;

    public void addToCache(String key, DataModel17 model) {
    ...
    public Optional<DataModel17> getFromCache(String key) {
        return Optional.ofNullable(cache.get(key));
    }

    public List<DataModel17> getAllCached() {
        return Collections.unmodifiableList(new ArrayList<>(cache.values()));
    }

    public List<DataModel17> filterByActive(boolean active) {
